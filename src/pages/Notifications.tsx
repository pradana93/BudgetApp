...
export default function Notifications() {
  ...
  const clearRead = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["notifications"] }); toast({ title: "Cleared read notifications" }); },
    onError: (e: Error) => toast({ title: "Failed", description: e.message, variant: "destructive" }),
  });
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">Notifications {unread > 0 && <Badge variant="pending" className="ml-2">{unread} unread</Badge>}</h1>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => markAll.mutate()} disabled={markAll.isPending || unread === 0}>Mark all read</Button>
          <Button variant="outline" size="sm" onClick={() => clearRead.mutate()} disabled={clearRead.isPending}>Clear read</Button>
        </div>
      </div>
      <Card><CardHeader><CardTitle>Recent activity</CardTitle></CardHeader><CardContent className="space-y-2">
        {isLoading && <div className="text-sm text-muted-foreground">Loading…</div>}
        {(data?.length ?? 0) === 0 && !isLoading && <div className="text-sm text-muted-foreground">No notifications yet — approvals, rejections and reconciliations will appear here.</div>}
        {data?.map((n) => (
